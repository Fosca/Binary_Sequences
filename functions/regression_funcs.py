...
sys.path.append("/Volumes/neurospin/meg/meg_tmp/ABSeq_Samuel_Fosca2019/scripts/ABSeq_PUBLICATION/")
...
def update_metadata_epochs_and_save_epochs(subject):
    ...
    # ---- for complexity and complexity squared to be less correlated, it is better to compute the regressor of complexity squared on the scaled complexity regressor
    from sklearn.preprocessing import scale
    complexity_squared = scale(metadata_notclean["Complexity"])**2
    ...
    print("---- Adding the metadata field Complexity squared -----")
    metadata_notclean = metadata_notclean.assign(Complexity_squared=complexity_squared)
...
def filter_good_epochs_for_regression_analysis(subject,clean=True,fields_of_interest = ['surprise_100','RepeatAlternp1']):
    ...
    if fields_of_interest is not None:
        for field in fields_of_interest:
            n_removed = np.sum(np.isnan(epochs.metadata[field].values))
            epochs = epochs[np.where(1 - np.isnan(epochs.metadata[field].values))[0]]
            print("--- removing %i epochs that have Nan values for field %s ----\n"%(n_removed,field))
...
def prepare_epochs_for_regression(subject, cleaned, epochs_fname, regressors_names, filter_name, remap_channels, apply_baseline, suffix):
    ...
    # LOAD EITHER THE EPOCHS OR THE RESIDUALS FROM PREVIOUS ANALYSIS
    if epochs_fname == '':
        print("---- filter the epochs removing the nans for the regressors -----")
        epochs = filter_good_epochs_for_regression_analysis(subject, clean=cleaned, fields_of_interest=regressors_names)
    ...
    # NORMALIZE (ZSCORE) THE REGRESSORS
    ...
    # SELECT THE HAB, STAND OR VIOL TRIALS
...
# ----------------------------------------------------------------------------------------------------------------------
def plot_betas_heatmaps(betas, ch_types, fig_path,suffix='',xlim = [-.050,.350]):
    ...
    for ch_type in ch_types:
        ...
        for x, regressor_name in enumerate(betas.keys()):
            # ---- Data
            evokeds = betas[regressor_name].average().copy().crop(tmin = xlim[0],tmax=xlim[1])
            if ch_type == 'eeg':
...
# ----------------------------------------------------------------------------------------------------------------------
def plot_betas_butterfly(betas, ch_types, fig_path,suffix='',xlim = [-.050,.350]):
    savepath = op.join(fig_path, 'Signals')
    utils.create_folder(savepath)
...
def merge_individual_regression_results(regressors_names, epochs_fname, filter_name,suffix = ''):
    ...
    for name in regressors_names:
        ...
        print(op.join(out_path, '" + name + suffix + "_epo.fif'))
...
def regression_group_analysis(regressors_names, epochs_fname, filter_name, suffix='', Do3Dplot=True, ch_types = ['mag'],suffix_evoked = ''):
    ...
    if filter_name == 'Viol':
        print("We are in the Viol case")
        xlim = [-.050, 0.600]
    else:
        print("We are in the Hab or Stand case")
        xlim = [-.050, 0.350]

    # ====================== PLOT THE GROUP-AVERAGED SOURCES OF THE BETAS  ===================== #
    if Do3Dplot:
        all_stcs, all_betasevoked = plot_average_betas_with_sources(betas, analysis_name, fig_path, xlim=[1000*xlim[0],1000*xlim[1]])

    # ================= PLOT THE HEATMAPS OF THE GROUP-AVERAGED BETAS / CHANNEL ================ #
    plot_betas_heatmaps(betas, ch_types, fig_path, suffix=suffix,xlim=xlim)

    # =========================== PLOT THE BUTTERFLY OF THE REGRESSORS ========================== #
    plot_betas_butterfly(betas, ch_types, fig_path, suffix=suffix,xlim=xlim)
